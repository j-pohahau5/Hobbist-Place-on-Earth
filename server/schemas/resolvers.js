    ...
    Mutation:{
        ...
        toggleLikeForHobby: async (_, { hobbyId, userId }) => {
            try {
                ...
                const userLikedIndex = hobby.likes.findIndex((like) => like.userId == userId);
                if (userLikedIndex !== -1) {
                    return await resolvers.Mutation.removeLikeFromHobby(_, { hobbyId, userId });  
                } else {
                    return await resolvers.Mutation.addLikeToHobby(_, { hobbyId, userId });
                }
            } catch (err) {
                throw new Error(err);
            }
        ...
        removeLikeFromHobby: async (_, { hobbyId, userId }) => {
            try {
                ...
                const userLikedIndex = hobby.likes.findIndex((like) => like.userId == userId);
                if (userLikedIndex === -1) throw new Error('User has not liked this hobby');
                ...
                if (hobbyIndex === -1) throw new Error('User does not have this hobby');if (hobbyIndex === -1) throw new Error('User does not have this hobby');
        ...
        removeCommentLike: async (_, { commentId, userId}) => {
            try {
                ...
                if (userLikedIndex === -1) throw new Error('User has not liked this comment');
        ...
        removeUser: async (_, { userId }) => {
            try {
                await Hobby.updateMany({ likes: { $eleMatch: { userId} } }, { $pull: {likes: { userId } } });
